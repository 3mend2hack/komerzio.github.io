...
// Estilos CSS para el modal mejorado (se inyectan automáticamente)
const styles = `
...
// Función para obtener/crear wallet del usuario
async function obtenerWalletUsuario() {
    ...
    const response = await fetch('https://sxxntcaobektiltyppwz.supabase.co/functions/v1/trondealer-create-invoice', {
        method: 'POST',
        headers: {
    ...
    if (!response.ok) {
        throw new Error(`Error ${response.status}: No se pudo obtener la wallet`);
    }
...
export const cryptoManager = {
    // MOSTRAR MODAL DE RECARGA - VERSIÓN SIMPLIFICADA (SIN PEDIR CANTIDAD)
    async mostrarModalRecarga() {
        ...
        try {
            // Obtener tasa actual
            const tasaResponse = await fetch('https://sxxntcaobektiltyppwz.supabase.co/functions/v1/get-usdt-rate');
            const tasaData = await tasaResponse.json();
            const tasa = tasaData.tasa || 320;
            
            // Obtener wallet del usuario (la crea si no existe)
            const walletData = await obtenerWalletUsuario();
            
            // Cerrar loading
            Swal.close();
            
            // Mostrar la wallet directamente
            Swal.fire({
                title: '💰 Recarga con USDT',
                html: `
                    <div class="crypto-modal-content">
                        <div class="crypto-tasa-box">
                            <i class="fa fa-chart-line" style="color: #f97316;"></i>
                            <strong>Tasa actual:</strong> 1 USDT = <span style="color: #f97316; font-size: 18px; font-weight: bold;">${tasa.toFixed(2)} CUP</span>
                        </div>
                        ...
                        <div style="margin-top: 20px; padding: 12px; background: #f0f9ff; border-radius: 12px;">
                            ...
                            <ul style="margin: 0; padding-left: 20px; text-align: left; font-size: 13px; color: #0c4a6e;">
                                ...
                                <li>Recibirás el equivalente en CUP según la tasa actual</li>
                                <li>La recarga es <strong>automática</strong> al confirmarse la transferencia</li>
                            </ul>
                        </div>
                        
                        <div class="crypto-info-text">
                            <i class="fa fa-clock-o"></i> La confirmación tarda ~3-5 minutos.<br>
                            Recibirás una notificación cuando se acredite tu saldo.
                        </div>
                    </div>
                `,
                icon: 'success',
            ...
            Swal.fire({
                icon: 'error',
                title: '❌ Error',
                text: error.message || 'Ocurrió un error al obtener tu wallet',
                confirmButtonColor: '#f97316'
            });
        }
    }
...
// Función para cargar saldo en header
...
console.log('✅ CryptoManager cargado - Modo simplificado (sin pedir cantidad)');